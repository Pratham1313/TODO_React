...
function Packinglist({ item, delete_list, done_task, clear_all }) {
  ...
  return (
    <main className="hello">
      ...
      {item.length > 0 ? <button onClick={clear_all}>Clear</button> : null}
      <ul>
        {item.map((item) => (
          <Item
            item={item}
...
function Stats({ items }) {
  if (items.length === 0) return <p>ADD Tasks</p>;
